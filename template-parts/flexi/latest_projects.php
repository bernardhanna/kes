 ...
 * - Defaults to the 3 most recent "project" posts.
 * - Optionally: editor can manually select exactly 3 projects.
...
$bg_color            = get_sub_field('background_color') ?: 'bg-gray-50';
$heading_color       = get_sub_field('heading_color') ?: 'text-blue-dark';
$text_color          = get_sub_field('text_color') ?: 'text-gray-800';
$accent_bar_color    = get_sub_field('accent_bar_color') ?: 'bg-blue-bright';
$card_rounded        = get_sub_field('card_rounded') ?: 'rounded-lg';
...
if ($use_manual && is_array($manual_projects) && count($manual_projects) === 3) {
    // Relationship field returned objects
...
<section id="<?php echo esc_attr($section_id); ?>" class="relative flex overflow-hidden <?php echo esc_attr($bg_color); ?>">
  <div class="flex flex-col items-center w-full mx-auto max-w-container pt-5 pb-5 max-lg:px-5<?php echo $padding_classes_str; ?>">
    ...
    <div class="w-full py-16 sm:py-20 lg:py-24">
        ...
        <!-- Header -->
        <div class="mb-12 lg:mb-16 flex flex-col lg:flex-row lg:justify-between lg:items-start gap-8">
          <div class="flex-1">
            <div class="mb-6">
              <h2 class="font-primary font-bold text-3xl sm:text-4xl lg:text-5xl <?php echo esc_attr($heading_color); ?> mb-3 leading-tight">
                <?php echo esc_html($heading); ?>
              </h2>
              <div class="w-8 h-1 <?php echo esc_attr($accent_bar_color); ?> rounded"></div>
            </div>

            <?php if (!empty($intro)): ?>
              <div class="wp_editor font-secondary font-normal text-base sm:text-lg leading-relaxed max-w-2xl <?php echo esc_attr($text_color); ?>">
          ...
          <?php if (!empty($cta_link) && is_array($cta_link)): ?>
            ...
            ?>
            <a href="<?php echo $cta_url; ?>" target="<?php echo $cta_target; ?>"
               class="flex items-center gap-2 px-4 py-4 rounded-full bg-white text-blue-dark font-secondary text-sm font-medium whitespace-nowrap hover:shadow-lg transition-all duration-300 h-fit">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
          <?php foreach ($projects as $p): ?>
            <?php
              ...
              // Description: prefer excerpt, fallback to trimmed content
            ...
            <article class="flex flex-col gap-6">
              <a class="block relative w-full <?php echo esc_attr($card_rounded); ?> overflow-hidden bg-gradient-to-r from-blue-dark to-blue-bright h-48 sm:h-56 lg:h-64" href="<?php echo $perma; ?>">
                <?php if ($img_url): ?>
                  <img class="w-full h-full object-cover" src="<?php echo $img_url; ?>" alt="<?php echo $img_alt; ?>" title="<?php echo $img_title; ?>">
                <?php endif; ?>
              </a>

              <div class="flex flex-col gap-3">
                <h3 class="font-secondary font-medium text-xl text-gray-800 leading-tight">
                  <a href="<?php echo $perma; ?>"><?php echo $title; ?></a>
                </h3>
                <?php if (!empty($excerpt)): ?>
                  <p class="font-secondary font-normal text-lg text-gray-700 leading-relaxed">
